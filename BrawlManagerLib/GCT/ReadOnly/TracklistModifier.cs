	...
	public class TracklistModifier {
        ...
        public byte this[int stageId] {
            get {
                ...
                if (id == stageId && this.StageIdMap.TryGetValue(id, out byte newStageId)) {
                    return newStageId;
                } else {
                    return id;
                }
                ...
                while (true) {
                    ...
                    if (ByteUtilities.ByteArrayEquals(line, 0, HEADER, 0, 7)) {
                        ...
                        for (int i = 0; i < lines; i++) {
                            ...
                            if (read < 8) throw new FormatException($"Could not read all of Tracklist Modifier code (line {i + 1}, read {read} bytes)");
